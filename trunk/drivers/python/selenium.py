...
### This part is hard-coded in the XSL
    def __init__(self, host, port, browserStartCommand, browserURL, Debug=False):
        self.host = host
        self.port = port
    ...
    def addCommand(self, name, values):
        """ add a new command to the selenium driver """
        self.__class__.__dict__[name] = new.instancemethod(self.executeCommand, values, self.__class__)

    def buildList(self):
        """ Build a list of selenium commands """
        
        def skip_command(command):
            skip = ("capture", "context", "open", "set", "wait", "window")
            for val in skip:
                if command.startswith(val):
                    return True
            return False
        
        self.Commands = [x for x in self.__class__.__dict__.keys() if not x.startswith("__")]
        andWaitCommands = []
        waitForCommands = []
        suffix = "AndWait"
        prefixes = ("waitFor_", "waitForNot_")
        for command in self.Commands: #: scan commands and separate waitfors from andWaits
            
            if skip_command(command):
                 continue #: skip this command
            
            if command.endswith("AndWait"):
                return #: ensure we're not rebuilding list
            
            if command.startswith("is"):
                waitForCommands.append(command)
            else:
                andWaitCommands.append(command)
        for command in andWaitCommands:
            self.addCommand("%s%s" % (command, suffix), (command, "", suffix))
        for command in waitForCommands:
            for prefix in prefixes:
                self.addCommand("%s%s" % (prefix, command), (command, prefix, ""))           
    
    def executeCommand(self, which, *args):
        """ Execute a dynamic Command  """
        command, prefix, suffix =  which
        timeout = None
        if len(args) > 1 and isInt(args[-1]):
            timeout = self.get_speed()
            self.set_timeout(int(args[-1]))
            args = args[:-1]
        if command in self.Commands:
            self.Prefix = prefix
            self.Suffix = suffix
            command = "self.%s%s " % (command, str(args))
            try:
                a = eval(command)
    ...
    def connect(self, reconnect=False):
        ...
        if not self.connection:
            raise Exception, "Unable to connect to server %s:%s" % (self.host, self.port)
    ...
    def do_command(self, verb, args):
        ...
        if verb.find("_is") > 1:
            verb = verb.replace("_is", "", 1)
        
        commandString = u'/selenium-server/driver/?cmd=%s' % urllib.quote_plus(unicode(verb).encode('utf-8'))
        for i in range(len(args)):
            commandString += "&%s=%s" % (unicode(i+1) , urllib.quote_plus(unicode(args[i]).encode('utf-8')))
            
        if not self.sessionString and not self.sessionId is None:
            self.sessionString  = "&sessionId=%s" % unicode(self.sessionId)
       
        commandString += self.sessionString
		
        if self.Debug:
            commandString = urllib.unquote_plus(commandString)
            print commandString[:commandString.rfind("&sess")]
        
        self.stopwatch()
        while True:
            try:
                self.connection.request("GET", commandString)
        ...
        if self.Debug:
            print response.status, response.reason, "Selenium Result: " + repr(data) + "\n"
        
        if not data.startswith("OK"):
            self.lastResult = (commandString, data)
        ...
        self.lastResult = (commandString, result)
        if not status == 'OK':
            raise Exception, result
    ...
    def get_string(self, verb, args):
        """ return the string from a command """
        return self.do_command(verb, args)
    
    def get_string_array(self, verb, args):
        """ return an array or strings, from the values returned from a command """
        tokens = []
        for row in csv.reader([self.get_string(verb, args)], escapechar="\\"):
            tokens.extend(row)
        
        return tokens

    def get_number(self, verb, args):
        """ return an integer from a command """
        try:
    ...
    def get_number_array(self, verb, args):
        ...
        tokens = []
        values = self.get_string_array(verb, args)
        for value in values:
            try:
    ...
    def get_boolean(self, verb=None, args=None, boolstr=None):
        """ check for boolean answer"""
        if not boolstr:
            boolstr = self.get_string(verb, args)
        values = {'true':True, 'false':False}
        if boolstr in values:
            return values[boolstr]
        
        return None
  
    def get_boolean_array(self, verb, args):
        """ check array for boolean anwsers """
        boolarr = self.get_string_array(verb, args)
        for i in range(len(boolarr)):
            boolarr[i] = self.get_boolean(boolstr=boolarr[i])
        return boolarr

####  Flex commands  ####

    def flexClick(self, locator):
        self.do_command("flexClick",  [locator, ])
        sleep(2)

    def flexMouseUp (self, locator):
        self.do_command("flexMouseUp",  [locator, ])
        sleep(2)

    def flexMouseDown (self, locator):
        self.do_command("flexMouseDown",  [locator, ])
        sleep(2)

    def flexMouseMove (self, locator, args):
        self.do_command("flexMouseMove",  [locator, args ])
        sleep(2)
    
    def flexDragTo (self, locator, x, y):
        self.do_command("flexDragTo",  [locator, "%s,%s" % (x, y)])
        sleep(2)

    def getFlexGlobalPostion(self, locator):
        pos = self.get_string("getFlexGlobalPosition",  [locator, ])
        return DictObj(dict(zip(('x','y'), [int(x) for x in pos.split(',')])))
    
    def getFlexComponentInfo(self, locator):
        info = self.get_string("getFlexComponentInfo",  [locator, ])
        return DictObj(dict(zip(('x','y', 'w','h'), [int(x) for x in info.split(',')])))
    
    def getFlexProperty(self, locator):
        return self.get_string("getFlexProperty",  [locator, ])
    
    def getFlexASProperty(self, locator):
        return self.get_string("getFlexASProperty",  [locator, ])
    
    def flexSetFlexObjID(self, locator):
        self.do_command("flexSetFlexObjID", locator)
    
    def flexType(self, locator, text):
        self.do_command("doFlexType", locator, text)
...
### From here on, everything's auto-generated from XML
    ...
    def click(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sclick%s" % (prefix, suffix),  [locator, ])
    ...
    def double_click(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdoubleClick%s" % (prefix, suffix),  [locator, ])
    ...
    def context_menu(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scontextMenu%s" % (prefix, suffix),  [locator, ])
    ...
    def click_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sclickAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def double_click_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdoubleClickAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def context_menu_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scontextMenuAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def fire_event(self, locator, eventName):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sfireEvent%s" % (prefix, suffix),  [locator, eventName, ])
    ...
    def focus(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sfocus%s" % (prefix, suffix),  [locator, ])
    ...
    def key_press(self, locator, keySequence):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%skeyPress%s" % (prefix, suffix),  [locator, keySequence, ])
    ...
    def shift_key_down(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sshiftKeyDown%s" % (prefix, suffix),  [])
    ...
    def shift_key_up(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sshiftKeyUp%s" % (prefix, suffix),  [])
    ...
    def meta_key_down(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smetaKeyDown%s" % (prefix, suffix),  [])
    ...
    def meta_key_up(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smetaKeyUp%s" % (prefix, suffix),  [])
    ...
    def alt_key_down(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%saltKeyDown%s" % (prefix, suffix),  [])
    ...
    def alt_key_up(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%saltKeyUp%s" % (prefix, suffix),  [])
    ...
    def control_key_down(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scontrolKeyDown%s" % (prefix, suffix),  [])
    ...
    def control_key_up(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scontrolKeyUp%s" % (prefix, suffix),  [])
    ...
    def key_down(self, locator, keySequence):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%skeyDown%s" % (prefix, suffix),  [locator, keySequence, ])
    ...
    def key_up(self, locator, keySequence):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%skeyUp%s" % (prefix, suffix),  [locator, keySequence, ])
    ...
    def mouse_over(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseOver%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_out(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseOut%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_down(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseDown%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_down_right(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseDownRight%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_down_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseDownAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def mouse_down_right_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseDownRightAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def mouse_up(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseUp%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_up_right(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseUpRight%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_up_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseUpAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def mouse_up_right_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseUpRightAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def mouse_move(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseMove%s" % (prefix, suffix),  [locator, ])
    ...
    def mouse_move_at(self, locator, coordString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%smouseMoveAt%s" % (prefix, suffix),  [locator, coordString, ])
    ...
    def type(self, locator, value):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%stype%s" % (prefix, suffix),  [locator, value, ])
    ...
    def type_keys(self, locator, value):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%stypeKeys%s" % (prefix, suffix),  [locator, value, ])
    ...
    def set_speed(self, value):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssetSpeed%s" % (prefix, suffix),  [value, ])
    ...
    def get_speed(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetSpeed%s" % (prefix, suffix),  [])
    ...
    def check(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scheck%s" % (prefix, suffix),  [locator, ])
    ...
    def uncheck(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%suncheck%s" % (prefix, suffix),  [locator, ])
    ...
    def select(self, selectLocator, optionLocator):
        ...
        *   \ **index**\ =\ *index*:
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sselect%s" % (prefix, suffix),  [selectLocator, optionLocator, ])
    ...
    def add_selection(self, locator, optionLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%saddSelection%s" % (prefix, suffix),  [locator, optionLocator, ])
    ...
    def remove_selection(self, locator, optionLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sremoveSelection%s" % (prefix, suffix),  [locator, optionLocator, ])
    ...
    def remove_all_selections(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sremoveAllSelections%s" % (prefix, suffix),  [locator, ])
    ...
    def submit(self, formLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssubmit%s" % (prefix, suffix),  [formLocator, ])
    ...
    def open(self, url):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sopen%s" % (prefix, suffix),  [url, ])
    ...
    def open_window(self, url, windowID):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sopenWindow%s" % (prefix, suffix),  [url, windowID, ])
    ...
    def select_window(self, windowID):
        ...
        *   \ **var**\ =\ *variableName*:
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sselectWindow%s" % (prefix, suffix),  [windowID, ])
    ...
    def select_frame(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sselectFrame%s" % (prefix, suffix),  [locator, ])
    ...
    def get_whether_this_frame_match_frame_expression(self, currentFrameString, target):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sgetWhetherThisFrameMatchFrameExpression%s" % (prefix, suffix),  [currentFrameString, target, ])
    ...
    def get_whether_this_window_match_window_expression(self, currentWindowString, target):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sgetWhetherThisWindowMatchWindowExpression%s" % (prefix, suffix),  [currentWindowString, target, ])
    ...
    def wait_for_pop_up(self, windowID, timeout):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%swaitForPopUp%s" % (prefix, suffix),  [windowID, timeout, ])
    ...
    def choose_cancel_on_next_confirmation(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%schooseCancelOnNextConfirmation%s" % (prefix, suffix),  [])
    ...
    def choose_ok_on_next_confirmation(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%schooseOkOnNextConfirmation%s" % (prefix, suffix),  [])
    ...
    def answer_on_next_prompt(self, answer):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sanswerOnNextPrompt%s" % (prefix, suffix),  [answer, ])
    ...
    def go_back(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sgoBack%s" % (prefix, suffix),  [])
    ...
    def refresh(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%srefresh%s" % (prefix, suffix),  [])
    ...
    def close(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sclose%s" % (prefix, suffix),  [])
    ...
    def is_alert_present(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisAlertPresent%s" % (prefix, suffix),  [])
    ...
    def is_prompt_present(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisPromptPresent%s" % (prefix, suffix),  [])
    ...
    def is_confirmation_present(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisConfirmationPresent%s" % (prefix, suffix),  [])
    ...
    def get_alert(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetAlert%s" % (prefix, suffix),  [])
    ...
    def get_confirmation(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetConfirmation%s" % (prefix, suffix),  [])
    ...
    def get_prompt(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetPrompt%s" % (prefix, suffix),  [])
    ...
    def get_location(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetLocation%s" % (prefix, suffix),  [])
    ...
    def get_title(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetTitle%s" % (prefix, suffix),  [])
    ...
    def get_body_text(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetBodyText%s" % (prefix, suffix),  [])
    ...
    def get_value(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetValue%s" % (prefix, suffix),  [locator, ])
    ...
    def get_text(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetText%s" % (prefix, suffix),  [locator, ])
    ...
    def highlight(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%shighlight%s" % (prefix, suffix),  [locator, ])
    ...
    def get_eval(self, script):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetEval%s" % (prefix, suffix),  [script, ])
    ...
    def is_checked(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisChecked%s" % (prefix, suffix),  [locator, ])
    ...
    def get_table(self, tableCellAddress):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetTable%s" % (prefix, suffix),  [tableCellAddress, ])
    ...
    def get_selected_labels(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetSelectedLabels%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_label(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetSelectedLabel%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_values(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetSelectedValues%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_value(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetSelectedValue%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_indexes(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number_array("%sgetSelectedIndexes%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_index(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetSelectedIndex%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_ids(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetSelectedIds%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_selected_id(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetSelectedId%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def is_something_selected(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisSomethingSelected%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_select_options(self, selectLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetSelectOptions%s" % (prefix, suffix),  [selectLocator, ])
    ...
    def get_attribute(self, attributeLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetAttribute%s" % (prefix, suffix),  [attributeLocator, ])
    ...
    def is_text_present(self, pattern):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisTextPresent%s" % (prefix, suffix),  [pattern, ])
    ...
    def is_element_present(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisElementPresent%s" % (prefix, suffix),  [locator, ])
    ...
    def is_visible(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisVisible%s" % (prefix, suffix),  [locator, ])
    ...
    def is_editable(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisEditable%s" % (prefix, suffix),  [locator, ])
    ...
    def get_all_buttons(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAllButtons%s" % (prefix, suffix),  [])
    ...
    def get_all_links(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAllLinks%s" % (prefix, suffix),  [])
    ...
    def get_all_fields(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAllFields%s" % (prefix, suffix),  [])
    ...
    def get_attribute_from_all_windows(self, attributeName):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAttributeFromAllWindows%s" % (prefix, suffix),  [attributeName, ])
    ...
    def dragdrop(self, locator, movementsString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdragdrop%s" % (prefix, suffix),  [locator, movementsString, ])
    ...
    def set_mouse_speed(self, pixels):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssetMouseSpeed%s" % (prefix, suffix),  [pixels, ])
    ...
    def get_mouse_speed(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetMouseSpeed%s" % (prefix, suffix),  [])
    ...
    def drag_and_drop(self, locator, movementsString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdragAndDrop%s" % (prefix, suffix),  [locator, movementsString, ])
    ...
    def drag_and_drop_to_object(self, locatorOfObjectToBeDragged, locatorOfDragDestinationObject):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdragAndDropToObject%s" % (prefix, suffix),  [locatorOfObjectToBeDragged, locatorOfDragDestinationObject, ])
    ...
    def window_focus(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%swindowFocus%s" % (prefix, suffix),  [])
    ...
    def window_maximize(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%swindowMaximize%s" % (prefix, suffix),  [])
    ...
    def get_all_window_ids(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAllWindowIds%s" % (prefix, suffix),  [])
    ...
    def get_all_window_names(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAllWindowNames%s" % (prefix, suffix),  [])
    ...
    def get_all_window_titles(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetAllWindowTitles%s" % (prefix, suffix),  [])
    ...
    def get_html_source(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetHtmlSource%s" % (prefix, suffix),  [])
    ...
    def set_cursor_position(self, locator, position):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssetCursorPosition%s" % (prefix, suffix),  [locator, position, ])
    ...
    def get_element_index(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetElementIndex%s" % (prefix, suffix),  [locator, ])
    ...
    def is_ordered(self, locator1, locator2):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisOrdered%s" % (prefix, suffix),  [locator1, locator2, ])
    ...
    def get_element_position_left(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetElementPositionLeft%s" % (prefix, suffix),  [locator, ])
    ...
    def get_element_position_top(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetElementPositionTop%s" % (prefix, suffix),  [locator, ])
    ...
    def get_element_width(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetElementWidth%s" % (prefix, suffix),  [locator, ])
    ...
    def get_element_height(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetElementHeight%s" % (prefix, suffix),  [locator, ])
    ...
    def get_cursor_position(self, locator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetCursorPosition%s" % (prefix, suffix),  [locator, ])
    ...
    def get_expression(self, expression):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetExpression%s" % (prefix, suffix),  [expression, ])


    def get_xpath_list(self, xpath):
        """
        Gets a full list of xpaths that match the given selector 
        
        'xpath' is the element to search for
        """
        prefix, suffix = ("", "")
        if self.Prefix: 
            prefix = self.Prefix 
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string_array("%sgetFullXpaths%s" % (prefix, suffix),  [xpath, ])
    ...
    def get_xpath_count(self, xpath):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_number("%sgetXpathCount%s" % (prefix, suffix),  [xpath, ])
    ...
    def assign_id(self, locator, identifier):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sassignId%s" % (prefix, suffix),  [locator, identifier, ])
    ...
    def allow_native_xpath(self, allow):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sallowNativeXpath%s" % (prefix, suffix),  [allow, ])
    ...
    def ignore_attributes_without_value(self, ignore):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%signoreAttributesWithoutValue%s" % (prefix, suffix),  [ignore, ])
    ...
    def wait_for_condition(self, script, timeout):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%swaitForCondition%s" % (prefix, suffix),  [script, timeout, ])
    ...
    def set_timeout(self, timeout):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssetTimeout%s" % (prefix, suffix),  [timeout, ])
    ...
    def wait_for_page_to_load(self, timeout):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%swaitForPageToLoad%s" % (prefix, suffix),  [timeout, ])
    ...
    def wait_for_frame_to_load(self, frameAddress, timeout):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%swaitForFrameToLoad%s" % (prefix, suffix),  [frameAddress, timeout, ])
    ...
    def get_cookie(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetCookie%s" % (prefix, suffix),  [])
    ...
    def get_cookie_by_name(self, name):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%sgetCookieByName%s" % (prefix, suffix),  [name, ])
    ...
    def is_cookie_present(self, name):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_boolean("%sisCookiePresent%s" % (prefix, suffix),  [name, ])
    ...
    def create_cookie(self, nameValuePair, optionsString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%screateCookie%s" % (prefix, suffix),  [nameValuePair, optionsString, ])
    ...
    def delete_cookie(self, name, optionsString):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdeleteCookie%s" % (prefix, suffix),  [name, optionsString, ])
    ...
    def delete_all_visible_cookies(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sdeleteAllVisibleCookies%s" % (prefix, suffix),  [])
    ...
    def set_browser_log_level(self, logLevel):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssetBrowserLogLevel%s" % (prefix, suffix),  [logLevel, ])
    ...
    def run_script(self, script):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%srunScript%s" % (prefix, suffix),  [script, ])
    ...
    def add_location_strategy(self, strategyName, functionDefinition):
        ...
        *   inDocument: the currently selected document
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%saddLocationStrategy%s" % (prefix, suffix),  [strategyName, functionDefinition, ])
    ...
    def capture_entire_page_screenshot(self, filename, kwargs):
        ...
        *    background
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scaptureEntirePageScreenshot%s" % (prefix, suffix),  [filename, kwargs, ])
    ...
    def set_context(self, context):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%ssetContext%s" % (prefix, suffix),  [context, ])
    ...
    def attach_file(self, fieldLocator, fileLocator):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sattachFile%s" % (prefix, suffix),  [fieldLocator, fileLocator, ])
    ...
    def capture_screenshot(self, filename):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%scaptureScreenshot%s" % (prefix, suffix),  [filename, ])
    ...
    def capture_screenshot_to_string(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        return self.get_string("%scaptureScreenshotToString%s" % (prefix, suffix),  [])
    ...
    def shut_down_selenium_server(self):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%sshutDownSeleniumServer%s" % (prefix, suffix),  [])
    ...
    def key_down_native(self, keycode):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%skeyDownNative%s" % (prefix, suffix),  [keycode, ])
    ...
    def key_up_native(self, keycode):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%skeyUpNative%s" % (prefix, suffix),  [keycode, ])
    ...
    def key_press_native(self, keycode):
        ...
        if self.Suffix: 
            suffix = self.Suffix 
        self.do_command("%skeyPressNative%s" % (prefix, suffix),  [keycode, ])
